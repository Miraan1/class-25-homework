...
function keyPressed() {
  ...
  if(keyCode === 32){
    ...
    var bowX = playerArcher.body.position.x
    var bowY = playerArcher.body.position.y
    var bowAngle = playerArcher.body.angle + PI/2
    var arrow = new PlayerArrow(bowX,bowY,100,10)
    arrow.trajectry = []
    Matter.Body.setAngle(arrow.body,bowAngle)
...
function keyReleased () {
  ...
  if(keyCode === 32){
    ...
    if(playerArrow.length){
      playerArrow[playerArrow.length-1].shoot(playerArcher.body.angle + PI/2)
    }
